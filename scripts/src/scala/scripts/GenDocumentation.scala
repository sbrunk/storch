...
import bleep.{BleepCacheLogger, BleepScript, Commands, FetchNode, PathOps, Started, constants, model}
...
object GenDocumentation extends BleepScript("GenDocumentation") {
  override def run(started: Started, commands: Commands, args: List[String]): Unit = {
    ...
    val mdoc = new MdocPlugin(started, scriptsProject) {
      override def mdocIn: Path = started.buildPaths.buildDir / "storch-site-in"
      override def mdocOut: Path = started.buildPaths.buildDir / "storch-site" / "docs"
    }
    ...
    val docusaurus = new DocusaurusPlugin(
      website = started.buildPaths.buildDir / "storch-site",
      mdoc = mdoc,
      docusaurusProjectName = "storch-site",
      env = env,
